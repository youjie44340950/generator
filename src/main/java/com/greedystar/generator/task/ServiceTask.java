...
public class ServiceTask extends AbstractTask {
    ...
    public void run() throws IOException, TemplateException {
        ...
        serviceData.put("BasePackageName", ConfigUtil.getConfiguration().getPackageName().getService().split(" ")[1]);
        ...
        serviceData.put("description", ConfigUtil.getConfiguration().getDescription());
        ...
        String filePath = StringUtil.package2Path(ConfigUtil.getConfiguration().getPackageName().getService().replaceAll(" ","")) + StringUtil.package2Path(ConfigUtil.getConfiguration().getPath().getService());
        String fileName;
        if (StringUtil.isBlank(ConfigUtil.getConfiguration().getPath().getInterf())) { // 表示不生成Service接口文件
            ...
            fileName = className + "Handler.java";
            System.out.println("Generating " + className + "Handler.java");
        } else {
            serviceData.put("Impl", "Impl implements " + className + "Handler");
            serviceData.put("Override", "\n    @Override");
            serviceData.put("InterfaceImport", "import " + ConfigUtil.getConfiguration().getPackageName().getInterf().split(" ")[1] + ConfigUtil.getConfiguration().getPath().getInterf() + "." + className + "Handler;");
            fileName = className + "HandlerImpl.java";
            System.out.println("Generating " + className + "HandlerImpl.java");
        }
        // 生成Service文件
